...
1) Run anvil: `anvil --mnemonic $TEST_MNEMONIC

2) Run `FOUNDRY_PROFILE=production forge script script/DeployTestnet.s.sol --rpc-url ws:/localhost:8545 --broadcast`

### Deploy to sepolia testnet:

1) Set `deployer` is deployments.toml. You can configure additional accounts here (see [31337.address]).

2) Run `FOUNDRY_PROFILE=production forge script script/DeployTestnet.s.sol --rpc-url $SEPOLIA_RPC_URL --private-key $DEPLOYER_KEY`
