...
contract MorphoVaultV1IntegrationBadDebtTest is MorphoVaultV1IntegrationTest {
    ...
    function testBadDebtThroughSubmitMarketRemoval() public {
        ...
        assertEq(vault.totalAssets(), INITIAL_ON_MARKET0);
        assertEq(vault.previewRedeem(vault.balanceOf(address(this))), INITIAL_ON_MARKET0);

        vault.accrueInterest();

        assertEq(vault._totalAssets(), INITIAL_ON_MARKET0);
    }
    ...
    function testBadDebtThroughLiquidate() public {
        ...
        assertEq(vault.totalAssets(), INITIAL_ON_MARKET0, "totalAssets() != INITIAL_ON_MARKET0");

        vault.accrueInterest();

        assertEq(vault._totalAssets(), INITIAL_ON_MARKET0, "totalAssets() != INITIAL_ON_MARKET0");
    }
}
